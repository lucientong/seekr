...
- 🔍 **文本搜索** — 高性能正则表达式匹配
- 🧠 **语义搜索** — 基于 ONNX 的本地 Embedding + HNSW KNN 搜索，按语义查找代码
- 🌳 **AST 模式搜索** — 通过 Tree-sitter 匹配函数签名、结构体、类（如 `fn(*) -> Result`）
- ⚡ **混合模式** — 三种搜索通过倒数排名融合（RRF）组合，获得最佳结果
...
3. **嵌入器（Embedder）** — 使用 ONNX Runtime + all-MiniLM-L6-v2 生成向量嵌入
4. **索引（Index）** — 构建倒排文本索引 + HNSW 向量索引，持久化到磁盘
5. **搜索（Search）** — 文本正则、语义 KNN、AST 模式匹配，通过 RRF 融合
